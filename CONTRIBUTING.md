# Contributing to playgrounder

## What this is

playgrounder is a Flutter package: a design-system-agnostic playground for
previewing a component live beside the knobs that configure it, with presets
and an inspector.
...
removals as expensive. In particular, `PlaygroundStyle` is the single seam a
consuming design system depends on to dress the playground — grow it
deliberately.
...
```
feat(knobs): add a color knob
fix(inspector): keep the TabController out of a lazy initializer
```
...
The example under `example/` is a separate package, excluded from the root
analysis options and analyzed on its own by CI. It is analyze-only — it has no
tests and does not count toward the package's coverage gate.
...
| the `playgrounder: ^X.Y.Z` install snippet in `README.md` | everyone reading the package's front page |
